...
/**
 * Antigravity CLI agent harness: runs `agy --print` as a self-contained agent
 * turn and streams `stream-json` deltas back through the OpenClaw agent event
 * pipeline. The harness owns the agy subprocess lifecycle (spawn, stream
 * parse, timeout/abort tree kill) and maps terminal facts to OpenClaw attempt
 * terminals so core model fallback can take over on quota/rate/auth failures.
 */
...
import {
  buildAgyProjectId,
  getAgyAvailability,
  resolveAgyBinaryPath,
  runAgyPrint,
} from "./src/agy-client.js";
...
type AntigravityAgentHarnessOptions = {
  id?: string;
  label?: string;
...
function readPluginConfig(cfg: unknown): AntigravityPluginConfig {
  const record = (typeof cfg === "object" && cfg !== null ? cfg : {}) as Record<string, unknown>;
  return {
    binaryPath: typeof record.binaryPath === "string" ? record.binaryPath : undefined,
    sandbox: record.sandbox === true,
    projectPrefix:
      typeof record.projectPrefix === "string" && record.projectPrefix.trim()
 ...
 */
function mapThinkLevelToAgyEffort(thinkLevel: string | undefined): string | undefined {
  if (!thinkLevel) {
    return undefined;
  }
 ...
 */
export function createAntigravityAgentHarness(
  options: AntigravityAgentHarnessOptions = {},
): AgentHarnessV2 {
  const harnessRuntimeId = options.id?.trim().toLowerCase() || ANTIGRAVITY_HARNESS_RUNTIME_ID;
  const resolveAttemptConfig = (config: OpenClawConfig | undefined) =>
    resolvePluginConfigObject(config, "antigravity") ??
   ...
   */
  const buildAntigravityAttemptResult = (
    params: AgentHarnessAttemptParamsV2,
    terminal: ReturnType<typeof agentHarnessAttemptTerminal.normalize>,
    patch?: { assistantTexts?: string[] },
  ...
  const harness: AgentHarnessV2 = {
    ...
    runAttempt: async (params: AgentHarnessAttemptParamsV2): Promise<AgentHarnessAttemptResult> => {
      ...
      if (!binaryPath) {
        const error = new Error(
          "agy (Antigravity CLI) binary was not found on PATH or in default install locations. " +
        ...
        return buildAntigravityAttemptResult(
          params,
          agentHarnessAttemptTerminal.normalize({
            promptError: error,
      ...
      let terminal: ReturnType<typeof agentHarnessAttemptTerminal.normalize>;
      if (result.timedOut) {
        ...
        terminal = agentHarnessAttemptTerminal.normalize({
          promptError: new Error(
            `agy exited with code ${result.exitCode ?? "unknown"} without a streamed result`,
          ),
          promptErrorSource: "prompt",
        });
      } else {
